...
- 如果有总计行，依赖该对象的除了`a-radio`的渲染 watcher 之外，还有计算属性 watcher，所以这两个肯定会更新，`table`组件是否更新其实取决于组件本身（而非 `table` 里面的子组件）是否依赖该对象。在我的实际工作场景中，使用的是`ant-design-vue`的 table ，测试结果是三个组件都执行了 updated ，原因是它内部有相关依赖。
...
<!-- cjh todo -->
2. 方法2：vue 中有一个组件属性，可以让这种场景下 table 组件不重新渲染，忘记是啥了，想起来再更新
